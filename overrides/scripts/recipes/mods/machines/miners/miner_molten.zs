...
RecipeBuilder.newBuilder("miner_Molten", "miner_molten", speed, 0)//这两个"miner_basic"是机器的ID与配方ID，保持相同即可
    ...
    //接下来每两行为一组，一组对应一个物品
    .addItemOutput(<additions:lighteum_ore>)//定义物品ID
    .setChance(0.09f / 10.0f)//定义几率（比方说这个就是0.3÷10=3%）
    
    .addItemOutput(<additions:mangisite_ore>)
    .setChance(0.08f / 10.0f)
    .addItemOutput(<additions:oneo_ore>)
    .setChance(0.07f / 10.0f)
    .addItemOutput(<additions:xenidium_ore>)
    .setChance(0.06f / 10.0f)
    .addItemOutput(<additions:rapesesium_ore>)
    .setChance(0.03f / 10.0f)
    .addItemOutput(<additions:nether_harcadium_ore>)
