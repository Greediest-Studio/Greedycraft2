...
SummoningDirector.addSummonInfo(
    SummoningInfo.create()
        ...
        .addMob(MobInfo.create()
            .setMob("gct_mobs:weather_eyevil")
            .setCount(1)
            .setOffset(0,4,0)
            .setSpread(1,1,1)
            .setData({"Health": BEHOLDER_HP, "Attributes":[{"Name":"generic.maxHealth","Base": BEHOLDER_HP}]})
        )
        .addMob(MobInfo.create()
            .setMob("gct_mobs:weather_water_rod")
        ...
        .setMutator(function (attempt as SummoningAttempt) {
            ...
            if (!pass1) {
                ...
                attempt.message = LangUtil.translate("greedycraft.zensummoning.beholder.fail.1");
            } else if (!pass2) {
                attempt.success = false;
                attempt.message = LangUtil.translate("greedycraft.zensummoning.beholder.fail.2");
            } else if (attempt.world.getProvider().getDimensionID() != 425) {
                attempt.success = false;
                attempt.message = LangUtil.translate("greedycraft.zensummoning.beholder.fail.5");
            } else { 
                attempt.message = LangUtil.translate("greedycraft.zensummoning.beholder.success");
            }
        })
);
