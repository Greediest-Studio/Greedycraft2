...
SummoningDirector.addSummonInfo(
    SummoningInfo.create()
        ...
        .addMob(MobInfo.create()
            ...
            .setOffset(0,10,0)
            .setSpread(1,1,1)
            .setData({"Health": GHASTHER_HP, "Attributes":[{"Name":"generic.maxHealth","Base": GHASTHER_HP}]})
        )
        .addMob(MobInfo.create()
        ...
        .setMutator(function (attempt as SummoningAttempt) {
            ...
            if (!pass1) {
                ...
                attempt.message = LangUtil.translate("greedycraft.zensummoning.ghasther.fail.1");
            } else if (!pass2) {
                attempt.success = false;
                attempt.message = LangUtil.translate("greedycraft.zensummoning.ghasther.fail.2");
            } else if (attempt.world.getProvider().getDimensionID() != -1) {
                attempt.success = false;
                attempt.message = LangUtil.translate("greedycraft.zensummoning.ghasther.fail.5");
            } else {
                attempt.message = LangUtil.translate("greedycraft.zensummoning.ghasther.success");
            }
        })
);
