    ...
    SummoningInfo.create()
        ...
        .setMutator(function (attempt as SummoningAttempt) {
            ...
            if (!pass1) {
                ...
                attempt.message = LangUtil.translate("greedycraft.zensummoning.ironslime.fail.1");
            } else if (!pass2) {
                attempt.success = false;
                attempt.message = LangUtil.translate("greedycraft.zensummoning.ironslime.fail.2");
            } else if (attempt.world.getProvider().getDimensionID() != 0) {
                attempt.success = false;
                attempt.message = LangUtil.translate("greedycraft.zensummoning.ironslime.fail.5");
            } else {
                attempt.message = LangUtil.translate("greedycraft.zensummoning.ironslime.success");
            }
        })
);
