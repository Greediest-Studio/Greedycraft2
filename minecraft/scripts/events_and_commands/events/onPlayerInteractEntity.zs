...
events.onPlayerInteractEntity(function(event as PlayerInteractEntityEvent) {
    ...
    if (event.target instanceof IEntityLivingBase && !event.world.isRemote()) {
        ...
        if (!isNull(player.mainHandHeldItem)) {
            ...
            //Spaceshock trait
            if (TicTool.isTool(item) && TicTool.hasTrait(item, "spaceshock") && player.getCooldown(item) <= 0.0f) {
                if (distance2D(player.x as double, player.z as double, target.x as double, target.z as double) >= 3.5d) {
                    ...
                    if (TicTool.hasTrait(item, "power_of_herrscher")) baseRange = 3;
            ...
            //Soul Flag trait
            if (TicTool.isTool(item) && TicTool.hasTrait(item, "soul_flag")) {
